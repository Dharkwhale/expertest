...
| `src/app/(prompt)/layout.tsx`, `(prompt)/live/{pocket,prompts/light,prompts/blue,prompts/blue/capture,pulse}` | S14–S17 full-screen live screens | no nav at all; each screen has its own back/✕ (`live/PromptNav`) to `/live`; centred `max-w-md` on desktop (§6.7) |
...
| `src/components/live/` | M4: `SpaceRow`, `ReactionPicker`, `NotificationCard` (S12). M5: `PromptNav`, `CaptureScreen` + `CapturePrompt` (S15 and S16's capture step), `CountdownRing` | |
